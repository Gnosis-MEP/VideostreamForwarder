...
python $QUERY_VIDEO_STREAMER $QUERY_ID $VIDEO_OUTPUT_TYPE | $FFMPEG_BIN -f rawvideo -r $FPS -pixel_format bgr24 -video_size $VIDEO_SIZE -i - -vf format=yuv420p -c:v libx264 -g 1 -x264opts "keyint=1:min-keyint=1:no-scenecut" -an -f flv ${OUPUT_MEDIA_SERVER_URL}$QUERY_ID